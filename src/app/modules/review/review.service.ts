...
import {  ICreateReviewPayload, IUpdateReviewPayload } from "./review.interface"
 ...
 const giveReview= async(user:any,payload:ICreateReviewPayload)=>{
      ...
      if(isReviewed){
       ...
       const result= await prisma.$transaction(async(tx)=>{
         const review= await tx.review.create({
          ...
          const avgRatting= await tx.review.aggregate({
            where:{
                doctorId:appointmentData.doctorId
            },
            _avg:{
           ...
           await tx.doctor.update({
            ...
            data: {
                averageRating: avgRatting._avg.rating as number
            }
        });
 ...
 const myReviews= async(user:any)=>{
...
await prisma.review.findMany({
 ...
 const updateReview= async(user:any,reviewId:string,payload:IUpdateReviewPayload)=>{
   const patientData = await prisma.patient.findUniqueOrThrow({
        where: {
            email: user?.email
        }
