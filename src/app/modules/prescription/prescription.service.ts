...
const givePrescription = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const filleName = `Prescription-${Date.now()}.pdf`;
    const uploadFille = await uploadToCloudinary(pdfBuffer, filleName);
    const pdfUrl = uploadFille?.secure_url;
    const uploadPrescription = await tx.prescription.update({
      where: {
        id: result.id,
      },
      data: {
...
const myPrescription = async (user: any) => {
...
const updatePrescription = async (user:any,prescriptionId: string, payload: any) => {
    ...
    if (!uploadedFile) {
        throw new AppError(status.INTERNAL_SERVER_ERROR, "Failed to upload PDF to Cloudinary");
    }
...
const deletePrescription = async (user: any, prescriptionId: string) => {
  const isUserExists = await prisma.user.findUniqueOrThrow({
    where: {
      email: user?.email,
    },
